...
public class Petugas_KonfirmasiBebasPustaka extends javax.swing.JFrame {
    ...
    private void toLoginMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_toLoginMouseClicked
        ...
        try {
            ...
            Logger.getLogger(Petugas_KonfirmasiBebasPustaka.class.getName()).log(Level.SEVERE, null, ex);
        }
        obj.setVisible(true);
        this.dispose();
                               ...
                               if(rsj.next()){
                                   ...
                                   if(numlg <= 0){
                                     int opt = JOptionPane.showConfirmDialog(null, "Siswa Yang bersangkutan memenuhi Syarat, Cetak Surat Bebas Pustaka?" , "Update", JOptionPane.YES_NO_OPTION);
                                        if(opt == 0){
                                                Statement stat1 = CC.createStatement();
                                                String sqla = "SELECT * FROM profile";
                                                ResultSet rsa = stat1.executeQuery(sqla);
                                                if (rsa.next()){
                                                    a = rsa.getString("profil");
                                                    b = rsa.getString("alamat");
                                                    img = rsa.getString("logo");
                                                }
                                                Statement stat2 = CC.createStatement();
                                                String sql = "SELECT * FROM anggota JOIN kelas ON anggota.IdKelas=kelas.IdKelas WHERE Nis='"+idt+"'";
                                                ResultSet rsb = stat2.executeQuery(sql); 
                                                if (rsb.next()){
                                                     nm = rsb.getString("Nama");
                                                     tk = rsb.getString("TingkatKelas");
                                                     jur = rsb.getString("IdJurusan");
                                                     kls = rsb.getString("kelas.Kelas");
                                                }
                                        
                                            String icon = "src/Uploads/Foto/Logo/"+img+"";
                                            String result = tk+" "+jur+" "+kls;
                                            HashMap param = new HashMap();
                                            param.put("instansi", a);
                                            param.put("alamat", b);
                                            param.put("nis", idt);
                                            param.put("nama", nm);
                                            param.put("kelas", result);
                                            param.put("img", icon);
                                            InputStream file = new FileInputStream(new File("src/Laporan/BebasPustaka.jrxml"));
